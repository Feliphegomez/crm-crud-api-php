...
<html>
	...
	<body>
		<script>
		// Esto se llama con los resultados de FG.getLoginStatus().
		...
		(function () {
			...
			s.async = true;
			s.src = 'https://crm.ltsolucion.com/sdk/';
			var x = document.getElementsByTagName('script')[0];
			x.parentNode.insertBefore(s, x);
		})();

		window.fgApiInit = function() {
			FG.init({
				// .. options Init
			});
			// Ahora que hemos inicializado el SDK de JavaScript, llamamos
			// FG.getLoginStatus(). Esta función obtiene el estado del
			...
			FG.getLoginStatus(function(response) {
				console.log('FG.getLoginStatus Front: ' + JSON.stringify(response));
				statusChangeCallback(response);
			});
		};
		...
		function testAPI(authResponse) {
			...
			document.getElementById('userData').innerHTML = JSON.stringify(FG.getUserInfo());
			
			Redoc.init(FG.initConfig().baseURL + '/openapi', {
			  scrollYOffset: 50
		...
		<!--
		  ...
		  the FG.login() function when clicked.
		-->

		<fg:login-button scope="email" onlogin="checkLoginState();">
		</fg:login-button>
